...
<script>
import teahcerApi from '~/api/teacher'
export default {
  asyncData(page) {
    console.log(page)
    return teahcerApi.getTeachInfo(page.route.params.id).then(resp => {
      return {
        teacher: resp.data.item.teacher,
        courseList: resp.data.item.courseList
      }
    })
